  ...
  <a-card :bordered="false">
    ...
    <a-spin :spinning="isLoading">
      <a-form :form="form" @submit="handleSubmit">
        ...
        <div v-show="$module('client-wxofficial')">
          <a-divider orientation="left">微信公众号授权登录</a-divider>
          <a-form-item label="一键授权登录/注册" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-radio-group v-decorator="['isOauthWxofficial', { rules: [{ required: true }] }]">
              <a-radio :value="1">
                <span>开启</span>
                <a-tag class="ml-5" color="green">推荐</a-tag>
              </a-radio>
            ...
            <div class="form-item-help">
              <p class="extra">开启后在微信公众号端一键获取用户授权并登录和注册（请先配置微信公众号设置）</p>
              <p
                v-show="form.getFieldValue('isOauthWxofficial') == 0"
                class="extra c-red"
              >关闭后微信公众号端将无法获取用户的openid，同时无法使用微信支付</p>
            </div>
          </a-form-item>

          <a-form-item
            v-show="form.getFieldValue('isOauthWxofficial') == 1"
            label="注册时绑定手机号"
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
          >
            <a-radio-group v-decorator="['isForceBindWxofficial', { rules: [{ required: true }] }]">
              <a-radio :value="1">
                <span>强制绑定</span>
                <a-tag class="ml-5" color="green">推荐</a-tag>
              </a-radio>
            ...
            <div class="form-item-help">
              <p class="extra">开启后在微信公众号一键授权注册时强制绑定手机号，仅首次注册时弹出</p>
              <p
                v-show="form.getFieldValue('isForceBindWxofficial') == 0"
                class="extra c-red"
              >如果不强制绑定手机号，会造成多端情况下同一个用户注册多个账户，强烈推荐绑定手机号</p>
            </div>
          </a-form-item>
        ...
        <div v-show="checkYoShopPlugin('mpAlipay')">
          <a-divider orientation="left">支付宝小程序授权登录</a-divider>
          <a-form-item label="一键授权登录/注册" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-radio-group v-decorator="['isOauthMpAlipay', { rules: [{ required: true }] }]">
              <a-radio :value="1">
                <span>开启</span>
                <a-tag class="ml-5" color="green">推荐</a-tag>
              </a-radio>
            ...
            <div class="form-item-help">
              <p class="extra">开启后在支付宝小程序端一键获取用户授权并登录和注册（请先配置支付宝小程序设置）</p>
              <p
                v-show="form.getFieldValue('isOauthMpAlipay') == 0"
                class="extra c-red"
              >关闭后支付宝小程序端将无法获取支付宝用户的buyerId，同时无法使用支付宝支付</p>
            </div>
          </a-form-item>

          <a-form-item
            v-show="form.getFieldValue('isOauthMpAlipay') == 1"
            label="注册时绑定手机号"
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
          >
            <a-radio-group v-decorator="['isForceBindMpAlipay', { rules: [{ required: true }] }]">
              <a-radio :value="1">
                <span>强制绑定</span>
                <a-tag class="ml-5" color="green">推荐</a-tag>
              </a-radio>
            ...
            <div class="form-item-help">
              <p class="extra">开启后在支付宝小程序端一键授权注册时强制绑定手机号，仅首次注册时弹出</p>
              <p
                v-show="form.getFieldValue('isForceBindMpAlipay') == 0"
                class="extra c-red"
              >如果不强制绑定手机号，会造成多端情况下同一个用户注册多个账户，强烈推荐绑定手机号</p>
            </div>
          </a-form-item>
...
export default {
  ...
  beforeCreate () {
    assignment(this, { isEmpty, checkYoShopPlugin })
  },
  // 初始化数据
  created () {
    // 获取当前详情记录
    ...
    setFieldsValue () {
      ...
      !isEmpty(form.getFieldsValue()) && $nextTick(() => {
        form.setFieldsValue(pick(record, [
          ...
          'isOauthWxofficial', 'isForceBindWxofficial',
          'isOauthMpAlipay', 'isForceBindMpAlipay'
        ]))
      })
    },
