<template>
  <div class="select-shop">
    <div class="data-preview clearfix">
      ...
      >
        <div v-for="(item, index) in selectedItems" :key="index" class="data-item">
          ...
          <a-tooltip>
            <template slot="title">
              <span class="f-12">{{ item.shop_name }}</span>
            </template>
    ...
    <div class="data-add">
      <a-button icon="plus" @click="handleSelectShop()">选择门店</a-button>
    </div>
    <ShopModal
      ref="ShopModal"
      :maxNum="maxNum"
      :defaultList="selectedItems"
      @handleSubmit="handleSelectShopSubmit"
    />
  </div>
</template>
...
// 仅需要的字段
const itemColumns = ['shop_id', 'shop_name', 'logo_url', 'phone', 'region', 'address']
...
export default {
  ...
  methods: {
    ...
    // 打开门店选择器
    handleSelectShop () {
      this.$refs.ShopModal.handle()
    },

    // 门店库modal确认回调
    handleSelectShopSubmit (result) {
