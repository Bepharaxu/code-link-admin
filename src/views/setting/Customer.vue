        ...
        <div v-if="record.enabled == 1">
          <a-form-model-item label="在线客服方式" :labelCol="labelCol" :wrapperCol="wrapperCol" required>
            ...
            <div v-show="record.provider === 'mpwxkf'" class="form-item-help">
              <p class="extra">仅支持微信小程序端，H5端、APP端等其他端不支持</p>
              <p class="extra">
                <span>网页版客服工具：</span>
                <a href="https://mpkf.weixin.qq.com" target="_blank">https://mpkf.weixin.qq.com</a>
              </p>
            </div>
            <div v-show="record.provider === 'wxqykf'" class="form-item-help">
              <p class="extra">支持微信小程序端、H5端、APP端</p>
              <p class="extra">
                <span>网页版客服工具：</span>
                <a
                  href="https://work.weixin.qq.com/kf/helpdesk"
                  target="_blank"
                >https://work.weixin.qq.com/kf/helpdesk</a>
              </p>
            </div>
          </a-form-model-item>
          <div v-if="record.provider === 'wxqykf'">
            <a-form-model-item
              label="客服链接"
              :labelCol="labelCol"
              :wrapperCol="wrapperCol"
              prop="config.wxqykf.url"
              :rules="{ required: true, message: '请填写客服链接' }"
            >
              <a-input v-model="record.config.wxqykf.url" />
            </a-form-model-item>
            <a-form-model-item
              label="企业ID"
              :labelCol="labelCol"
              :wrapperCol="wrapperCol"
              prop="config.wxqykf.corpId"
